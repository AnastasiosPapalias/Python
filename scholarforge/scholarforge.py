...
def print_banner() -> None:
    ...
    for line in logo_lines:
        # Centre the raw text inside the box
        raw = re.sub(r'\033\[[0-9;]*m', '', line)
        padding = max(0, w - 2 - len(raw))
        left_pad = padding // 2
        right_pad = padding - left_pad
        print("║" + " " * left_pad + line + " " * right_pad + "║")
    tagline = "Academic Research & Knowledge Pipeline  ·  v3.0.0"
    tag_pad = (w - 2 - len(tagline)) // 2
    print("║" + " " * tag_pad + _c(tagline, C.BRIGHT_WHITE, C.ITALIC) + " " * (w - 2 - tag_pad - len(tagline)) + "║")
    url_line = "github.com/AnastasiosPapalias/scholarforge"
    url_pad = (w - 2 - len(url_line)) // 2
    print("║" + " " * url_pad + _c(url_line, C.DIM) + " " * (w - 2 - url_pad - len(url_line)) + "║")
    print("╚" + "═" * (w - 2) + "╝")
    print()
...
def search_semantic_scholar(query: str, query_mode: str, subjects: List[str], limit: int = 20) -> List[HarvestRecord]:
    ...
    try:
        data  = request_with_retry(SEMANTIC_SCHOLAR, params, expect_json=True)
    except Exception:
        return []
...
def search_core(query: str, query_mode: str, subjects: List[str], max_results: int = 20) -> List[HarvestRecord]:
    ...
    try:
        data = request_with_retry(CORE_BASE, params, expect_json=True)
        out: List[HarvestRecord] = []
...
FORMATS (Forge)
  PDF · DOCX · TXT · Markdown · HTML · XML · JSON · CSV · TSV · RTF · YAML · TeX
    ...
    p.add_argument("--version", action="version", version="ScholarForge 3.0.0")
